...
const ageGroups = [
  ...
  { range: "8–11", label: "Primary", emoji: "📖", color: "#1E3888" },
    ...
    <div className="min-h-screen" style={{ fontFamily: "var(--font-nunito), sans-serif", background: "#F5F8FF" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4"
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3">
          <Link href="/login" className="px-4 py-2 rounded-xl font-bold transition-all" style={{ color: "#1E3888", fontWeight: 700 }}>
            Teacher Login
          </Link>
          <Link href="/login"
            className="px-5 py-2.5 rounded-xl text-white font-black text-sm transition-all hover:scale-105 hover:shadow-lg"
            style={{ background: "linear-gradient(135deg, #1E3888, #47A8BD)", fontWeight: 800 }}>
            Play &amp; Learn
          </Link>
        </motion.div>
      </nav>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }}
            ...
            style={{ background: "#EDE9FE", color: "#1E3888", fontWeight: 700 }}>
          ...
          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}
            className="text-6xl md:text-7xl font-black mb-6 leading-tight" style={{ color: "#000000" }}>
            Where Kids
            <span className="block" style={{ background: "linear-gradient(135deg, #1E3888, #F43F5E)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              Love to Learn
            </span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <Link href="/login"
              ...
              style={{ background: "linear-gradient(135deg, #1E3888, #47A8BD)", fontWeight: 900 }}>
              🎮 Start Playing Now
            </Link>
            <Link href="/backoffice"
              className="px-8 py-4 rounded-2xl font-black text-xl border-2 transition-all hover:scale-105 bg-white"
              style={{ color: "#1E3888", borderColor: "#1E3888", fontWeight: 900 }}>
              📊 Teacher Dashboard
            </Link>
          </motion.div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.h2 variants={fadeUp} className="text-4xl font-black text-center mb-3" style={{ color: "#000000" }}>
              Built for Every Age
            </motion.h2>
            ...
            <motion.div variants={stagger} className="grid sm:grid-cols-2 gap-6">
              {features.map((f) => (
                <motion.div key={f.title} variants={fadeUp}
                  ...
                  <h3 className="text-2xl font-black mb-2" style={{ color: "#000000" }}>{f.title}</h3>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.h2 variants={fadeUp} className="text-4xl font-black text-center mb-12" style={{ color: "#000000" }}>
              How TeachMore Works
            </motion.h2>
            <div className="grid sm:grid-cols-3 gap-8">
              {[
                { step: "1", title: "Set Up Your Classroom", desc: "Create a class, add students by age group, and pick subjects.", icon: "🏫", color: "#1E3888" },
                ...
                <motion.div key={item.step} variants={fadeUp} className="text-center">
                  ...
                  <h3 className="text-xl font-black mb-2" style={{ color: "#000000" }}>{item.title}</h3>
      ...
      <section className="py-24 px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          ...
          style={{ background: "linear-gradient(135deg, #1E3888 0%, #47A8BD 50%, #06B6D4 100%)" }}>
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <Link href="/student"
              ...
              style={{ background: "white", color: "#1E3888", fontWeight: 900 }}>
              🎮 Student Portal
            </Link>
            <Link href="/backoffice"
      ...
      <footer className="py-8 text-center text-gray-400 text-sm" style={{ borderTop: "1px solid #F0E8FF" }}>
        <span className="font-black" style={{ color: "#1E3888" }}>TeachMore</span> — Learn. Play. Grow.
      </footer>
    </div>
  );
}
